function [avgError,flag]=Test_tan_se()
...
%进行1000次的计算误差分析
  ...
  %将标准库中的tand()与编写的tan_se()运算值进行误差计算：
     %对每一次误差计算的操作：
          %(1)两者误差值大于设定标准误差，即tan_se()计算错误，flag++
          %(2)两者误差值大于设定标准误差，即tan_se()计算无误
     %如上的操作重复1000（T）次，最终误差计算=测试计算中发生错误的次数/总测试数
     
for i=1:T %比较，计算错误的个数
    b(i)=tand(a(i));%标准值
    c(i) = m_tan_se( a(i) );%测试值
    if abs(b(i))==Inf&&abs(c(i))==Inf %考虑a(i)取90的情况
         erro(i)=0;
     else
